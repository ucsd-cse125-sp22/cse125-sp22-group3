...
void Mesh::draw(glm::mat4 view, glm::mat4 projection, glm::mat4 parent, float time, GLuint shaderProgram) {
    if (isParticle) {
        particleDraw(view, projection, parent, time, shaderProgram);
    }
        ...
        glUniform1f(glGetUniformLocation(shaderProgram, "lightSize"), FBO::playerPos.size());
...
void Mesh::particleDraw(glm::mat4 view, glm::mat4 projection, glm::mat4 parent, float time, GLuint shader) {
    ...
    int index = int(time) % textures.size();
...
void Mesh::draw(glm::mat4 view, glm::mat4 projection, glm::mat4 parent, std::vector<glm::mat4> transforms, float time, GLuint shaderProgram) {
    // Actiavte the shader program 
    glUseProgram(shaderProgram);
